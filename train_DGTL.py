...
def train_DGTLGCN():
    ...
    time_vector = torch.zeros(sum_pos_index.size(1), len(train_data) - 1) #val_len 1
    ...
    time_vector = time_vector * decay_tensor
    ...
    best_map = 0
    best_auc = 0
    edge_index_train = sum_pos_index.to(device)
    for e in range(epoch):
        model.train()
        prob, time_vector_train = model((feature_set, adj_new_set), time_vector, edge_index_train)

        zero_vector = torch.zeros(1, emb_dim).float().to(device)
        time_vector_train = torch.cat((time_vector_train, zero_vector), 0)
        last_pos_time_vector_test = time_vector_train[last_time_vector_pos_index]
        last_neg_time_vector_test = time_vector_train[last_time_vector_neg_index]

        loss_cross = loss_new(prob * eta, last_pos_index, last_neg_index, time_vector_flag,
                              last_pos_time_vector_test * gama, last_neg_time_vector_test * gama)
        ...
        for i, t in enumerate(test_data):
            ...
            pos_time_vector_test = time_vector_train[pos_time_vector_index_list[i]]
            ...
            neg_time_vector_test = time_vector_train[neg_time_vector_index_list[i]]

            auc, ap = loss_new.predict(prob * eta, pos_index, neg_index, time_vector_flag, pos_time_vector_test * gama, neg_time_vector_test * gama)
            auc_list.append(auc)
            ap_list.append(ap)
        map = np.mean(ap_list)
